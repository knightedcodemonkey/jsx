...
4. **Bundle-size trims** – Audit shared helpers/metadata for duplication, experiment with feature flags (opt-out of advanced descriptors), and run esbuild/rollup analyzers to find dead code so the next releases can claw back some of the new bytes. Follow up by splitting diagnostics into an explicit opt-in (e.g., `@knighted/jsx/debug` or `?debug` subpaths) and offering an opt-out toggle/env flag for CDN users so production bundles can skip the dev helpers entirely.
